#!/usr/bin/env python3
"""Convert and optimize VibeVoice multi-speaker TTS models to CoreML.

Runs all conversion steps sequentially:
  1. Components (diffusion, VAE, encoders, connectors, lm_head)
  2. Stateful LM decoder (Qwen2.5 with KV cache)
  3. Streaming semantic encoder (causal σ-VAE with conv caches)
  4. Optimizations: INT8 quantization, fused LM+head (optional)

Usage:
    uv run python convert_all.py --model-id microsoft/VibeVoice-1.5B
    uv run python convert_all.py --model-id vibevoice/VibeVoice-7B --int8 --fuse-lm-head
    uv run python convert_all.py --model-id microsoft/VibeVoice-1.5B --skip-vae --skip-semantic
"""
...
SUPPORTED_MODELS = ["microsoft/VibeVoice-1.5B", "vibevoice/VibeVoice-7B"]
...
def create_fused_lm_head(output_path: Path, model_id: str, cfg: dict,
                         int8: bool = False):
    """Create a fused LM decoder + LM head CoreML model.

    Combines the decoder with the output projection into a single model.
    Outputs both hidden_states and logits, saving one dispatch per token.
    """
    import coremltools as ct
    ...
    # fall back to embed_tokens (tied weights)
    if "lm_head.weight" in all_weights:
        lm_head_weight = all_weights["lm_head.weight"].float()
...
@app.command()
def convert_all(
    model_id: str = typer.Option(..., "--model-id", help=f"Model ID: {SUPPORTED_MODELS}"),
    output_dir: Path = typer.Option(None, "--output-dir", help="Output directory (default: build/<model>)"),
    ...
    skip_streaming_semantic: bool = typer.Option(False, "--skip-streaming-semantic"),
    ...
    fused_diffusion: bool = typer.Option(False, "--fused-diffusion", help="Export fused diffusion loop"),
    ...
    if output_dir is None:
        output_dir = Path(__file__).resolve().parent.parent / f"build/{model_id.split('/')[-1].lower()}"
    t_total = time.time()
    ...
    if fused_diffusion:
        cmd += ["--fused-diffusion",
                "--fused-diffusion-steps", str(fused_diffusion_steps),
                "--fused-diffusion-cfg", str(fused_diffusion_cfg)]
    ...
    # 3. Streaming semantic encoder (needed for e2e feedback loop)
    if not skip_streaming_semantic:
        _run(
            ["convert_streaming_semantic.py", "--output-dir", str(output_dir)],
            "Streaming semantic encoder (convert_streaming_semantic.py)",
        )

    # 4. Optimizations
    from convert_stateful_lm import MODEL_CONFIGS
    cfg = MODEL_CONFIGS[model_id]

    if int8 and not fuse_lm_head:
        # Standalone INT8: quantize the base LM decoder
        print(f"\n{'=' * 70}")
    ...
    if fuse_lm_head:
        ...
        create_fused_lm_head(fused_path, model_id, cfg, int8=int8)

    # 5. Clean up intermediate models that are superseded by optimized variants
    import shutil
    cleanup = []
    if int8 and not fuse_lm_head:
        # --int8 only: keep int8, remove fp16 base
        cleanup.append("lm_decoder_stateful.mlpackage")
    if fuse_lm_head and not int8:
        # --fuse-lm-head only: keep fused, remove base + separate head
        cleanup += ["lm_decoder_stateful.mlpackage", "lm_head.mlpackage"]
    if fuse_lm_head and int8:
        # --int8 --fuse-lm-head: keep fused_int8, remove everything else
        cleanup += [
    ...
    if fused_diffusion:
        # fused diffusion loop supersedes standalone diffusion head
        cleanup.append("diffusion_head.mlpackage")
