#!/usr/bin/env python3
"""Convert VibeVoice multi-speaker LLM backbone to stateful CoreML.
...
Exports:
  ...
  - embed_tokens.bin — embedding table for token lookup (float16 binary)

Usage:
    uv run python convert_stateful_lm.py --model-id microsoft/VibeVoice-1.5B
    uv run python convert_stateful_lm.py --model-id vibevoice/VibeVoice-7B
"""
...
MODEL_CONFIGS = {
    "microsoft/VibeVoice-1.5B": {
        ...
        "rms_norm_eps": 1e-6,
        "default_max_seq": 65535,  # CoreML state tensor limit: 2^16-1
    },
...
class StatefulQwen2Decoder(nn.Module):
    """Qwen2.5 decoder with stateful KV cache for GPU execution.

    Uses standard Linear layers which CoreML maps to optimized GPU matmul.
    Benchmarked at ~8ms/token on M-series (CPU_AND_GPU). Conv1d variant was
    tested for ANE but variable-length KV cache attention forces mixed
    execution, resulting in ~50ms/token due to dispatch overhead.
    """
...
def main():
    ...
    parser.add_argument("--output-dir", default=None, help="Output directory (default: build/<model>)")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else _parent / f"build/{args.model_id.split('/')[-1].lower()}"
    ...
    # ---- Save embeddings before any model surgery ----
    ...
    # ---- Free PyTorch memory before CoreML conversion ----
    del stateful_model, text_model, layers, final_norm
    import gc
    gc.collect()
    ...
    # ---- Export embeddings as float16 binary ----
    ...
    f32_mb = embed_weights.nbytes / 1e6
    f16_mb = embed_path.stat().st_size / 1e6
    print(f"Embeddings: {embed_path} {embed_weights.shape} ({f16_mb:.1f}MB, was {f32_mb:.1f}MB as float32)")
