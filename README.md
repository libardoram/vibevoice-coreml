...
Benchmarked on M4 Max (64GB). RTF = real-time factor (higher is faster; >1x means faster than real-time).

| Model | Backend | RTF / Speed | Peak RAM (Unified) | GPU utilization | Power |
|-------|---------|-------------|----------|-----|-------|
| 0.5B | CoreML (Swift) | 5.4x RTF | **21 GB** | 74% | **29 W** |
| 0.5B | MLX (Python) | **5.9x** RTF | 33 GB | 82% | 45 W |
| 1.5B | CoreML (Swift) | 2.4x RTF | **30 GB** | 77% | **41 W** |
| 1.5B | MLX (Python) | **3.3x** RTF | 41 GB | 76% | 56 W |
| 7B | CoreML (Swift) | **1.2x** RTF | **41 GB** | 74% | **59 W** |
| 7B | MLX (Python) | 0.7x RTF | 55 GB | 61% | 61 W |
| ASR | CoreML (Swift) | 19.6 tok/s | **33 GB** | 85% | **68 W** |
| ASR | MLX (Python) | **35.0 tok/s** | 38 GB | 88% | 79 W |

### When to use MLX vs CoreML

- **ASR**: Use MLX. MLX is ~1.8x faster than CoreML (35 vs 20 tok/s). 
- **TTS 0.5B**: Both backends produce audio at comparable speed. CoreML uses 36% less RAM (21 vs 33 GB) and 36% less power. MLX is a good choice if you're already in a Python workflow.
- **TTS 1.5B / 7B**: Use CoreML. The MLX pipeline produces choppy audio regardless of quantization settings (probably fixable). MLX 7B is also slower than real-time (0.7x) and needs 55 GB RAM.
